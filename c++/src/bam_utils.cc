...
bool Utilities::FixReads(const string& name, BamAlnVector& als, int& type, int& n_total, vector<int32_t>& iss, BamAlnPairVector& alpairs) {
  ...
  if((int)als.size() > 2) {
    ...
    for(vector<int>::reverse_iterator rit = idxs_rm.rbegin(); rit < idxs_rm.rend(); rit ++)
  ...
  for(BamAlnPairVector::iterator it = alpairs.begin(); it != alpairs.end(); it ++) {
    ...
    int type1 = (ismapped1 && ismatemapped1) ? 1 : (!ismapped1 && !ismatemapped1) ? 3 : 2;
    int type2 = (ismapped2 && ismatemapped2) ? 1 : (!ismapped2 && !ismatemapped2) ? 3 : 2;
    if(type1 == type2) {
