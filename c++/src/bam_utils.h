...
class Utilities {
  public:
    static bool ParseRegionString(const string& regionString, const BamReader& reader, BamRegion& region);
    bool PairReads(const string& name, const BamAlnVector& als, int& n_total, BamAlnPairVector& alpairs);
  ...
  protected:
    static bool CheckReadPair(const BamAlignment& al1, const BamAlignment& al2, int32_t& is);
};
