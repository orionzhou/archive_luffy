...
sub guess_options {
  ...
  my ($exons,$utrs,$cds);
  foreach ($self->parts) {
    $exons++ if $_->feature->primary_tag =~ /exon/i;
    $utrs++  if $_->feature->primary_tag =~ /utr$/i;
    $cds++   if $_->feature->primary_tag =~ /^cds/i;
...
sub create_implied_utrs {
  ...
  my @exons   = grep {$_->primary_tag =~ /^exon/} @features;
  my @cds     = grep {$_->primary_tag =~ /^CDS/ } @features;
  my @old_utr = grep {$_->primary_tag =~ /UTR/  } @features;
...
sub adjust_exons {
  ...
  my @exon   = grep {$_->feature->primary_tag =~ /exon/i} @parts;
