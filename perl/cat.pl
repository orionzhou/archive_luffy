...
use Readfile; use Writefile; use GO::Parser; use Parser;
use Bio::Seq; use Path::Class; use Data::Dumper;
use List::MoreUtils qw/first_index last_index insert_after apply indexes pairwise zip uniq/;
#updateGoFiles();
my $refDb = "mt_35";
my $dirW = dir($DIR_Misc1, "cat", $refDb);

my $f01 = file($dirW, "01_gene_ipr.tbl");
#get_ipr_ids($refDb, $f01);
my $f02 = file($dirW, "02_gene_gos.tbl");
my $f03 = file($dirW, "03_gene_go.tbl");
#map_ipr_to_go($dirW, $f01, $f02, $f03);

sub get_ipr_ids {
    my ($refDb, $fo) = @_;
    my $fi = file($DIR_Genome, $refDb, "10_model/53.gtb");
