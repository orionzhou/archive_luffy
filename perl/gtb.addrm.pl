...
=head1 NAME
  ...
  gtb.addrm.pl - incorporate RepeatMasker result to Gtb

=head1 SYNOPSIS
  
  gtb.addrm.pl [-help] [-v overlap-bed]  [-in input-file] [-out output-file]

  Options:
    -h (--help)   brief help message
    -i (--in)     input file (Gtb format)
    -o (--out)    output file (Gtb format)
    -v (--ovl)    repeatmasker overlap file (Bed format)
...
pod2usage(2) if !$fi || !$fo || !$fv;
  
  open(my $fhv, $fv) || die "cannot read file $fv\n";
  my $h = {};
  while(<$fhv>) {
    chomp;
