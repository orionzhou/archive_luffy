...
sub get_sra_info {
    ...
    open(FHO, ">$fo") or die "cannot write $fo\n";
    print FHO join("\t", qw/uid n_run runs title platform experiment study taxid lib
        lib_strategy lib_layout/)."\n";
    my $fac = Bio::DB::EUtilities->new(-eutil=>'esummary', -email=>'zhoupenggeni@gmail.com', -db=>'sra', -id=>\@uids);
    my @its;
    my $xml;
    ...
    while(my $ds = $fac->next_DocSum) {
        ...
        print FHO join("\t", $uid, $n_run, $runs, 
            $title, $platform, $exp, $study, $taxid, $lib, $lib_strategy, $lib_layout)."\n";
    }
    close FHO;
}
sub download_sra {
