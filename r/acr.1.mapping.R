...
tb = within(tb, {
	start = start + 1
	size = stop - start + 1
	lid = sprintf("%d-%d-%d",chr, start, stop)
})
nrow(tb)
...
ti = read.table(fi, sep = "\t", header = T, as.is = T)[,1:20]
#ti = ti[ti$alnLen/ti$qSize >= 0.8 & ti$ident >= 0.8,]


tb3 = merge(tb2, ti, by.x = 'lid', by.y = 'qId', all.x = T)
tb4 = within(tb3, {
	pgdist = ifelse(is.na(pchr) | tId != pchr, NA,
...
tp = to[,c('chr','start','stop','summit','gid','gdist','category', 
